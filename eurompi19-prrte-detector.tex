...
\begin{abstract}
As the scale of High Performance Computing (HPC) system continues to grow, mean-time-to-failure (MTTF) of these HPC systems is negatively impacted and tend to decrease. In order to efficiently run long computing jobs on these systems, handling system failures become a prime challenge. We present here the design and implementation of an efficient runtime-level failure detection and propagation strategy targeting large scale, dynamic systems able to detect both node and process failures. Multiple, overlapping topologies are used to optimize the detection and propagation, minimizing the incurred overheads and guaranteeing the scalability of the entire framework.
