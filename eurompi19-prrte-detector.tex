...
\begin{figure}[h]
  ...
  \caption{Single Daemon Failure detection and propagation delay compared to \ulfm with different heartbeat periods.}
...
For the heartbeat period setting we start from 30 milliseconds to 0.5 second for both \ourwork and \ulfm. For all heartbeat periods we set $ \eta = \delta * 2 $. From the figure, we can see that the detection latency in all cases lands in the interval $[\delta,\eta]$.
