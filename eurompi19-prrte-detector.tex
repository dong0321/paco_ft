...
\title{Efficient Failure Detection and Propagation in Distribute Systems}
...
The experimental evaluation of the resulting software stack on different machines demonstrate that
...
\begin{keyword}
Fault tolerance \sep Failure detection \sep Reliable broadcast \sep Message propagation \sep HPC runtime system
\end{keyword}
...
\begin{equation}
Overhead = \frac{( {\ourwork - \prrte} )}{\prrte}\label{eq:1}
\end{equation}
...
To further validate the logarithmic trend of \ourwork scalability, we scales the evaluation on the larger Cori system (with more processes per node). We can see in Figure~\ref{fig:proc_failure_cori} that with 4K processes the detection and propagation latency is about 10 milliseconds, and the scalability trend remains logarithmic with the number of nodes (not processes).
...
Figure~\ref{fig:daemon_failure_diffnode} shows single node failure detection and propagation performance with a fixed heartbeat period $ \delta = 0.5s $ and an increasing total number of nodes. After a node crash, all processes hosted on this node will be affected, the observer node fetches and packs the information of all affected processes information, then distributes the packed message.
\textbf{
\begin{figure}[h]
  \centering
...
\section{Application support and evaluation}\label{sec:application}
In this section we investigate application support of \ourwork with different programming models
including Pmix, \mpi and OpenMP or even hybrid applications. Our interests focus on complex application which
associated with multiple programming models. Because for hybrid applications and models, there is no "standard" method by which programming models could
follow to initiate themselves, it will be hard to inform each programming model
that another model is actively in use.
...
Also, OpenMP does not have an explicit call to "init" and is instead initialized on first use.
Figure~\ref{fig:prrte_with_oshmem} shows how to coordinate between those two different models. We can see
that they share the same external PMIx library within the processes, it will therefore be common to both programming
libraries. Thus, the calls into PMIx\_Init by each programming model will enter the same code space.
And then, the event notification mechanism within PMIx will share the information and coordinate between those models.
\prrte supports different type of applications by: first use \textbf{\emph{prte}} to start the distributed
virtual machine (DVM) and then use \textbf{\emph{prun}} to execute the binary as long as they are compiled correspondingly as:
\begin{enumerate}
  \item PMIx-based application use \textbf{\emph{pcc}} for compilation;
  \item MPI application need to install \mpi and \ourwork with the same external PMIx, then use \textbf{\emph{mpicc}} for compilation;
  \item OpenSHMEM application need to install \mpi (with option \textbf{\emph{-enable-shmem}}) and
  also \ourwork with the same external PMIx, then use \textbf{\emph{oshcc}} for compilation;
\end{enumerate}

\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{prrte_with_oshmem.pdf}\vspace{-1em}
  \caption{Hybrid programming model support of MPI and OpenSHMEM}
  \label{fig:prrte_with_oshmem}
\end{figure}

We also evaluate the overhead performance of \ourwork
with MPI and OpenSHMEM applications using the heavily communication-bound benchmark Graph500~\cite{graph500}.
Graph500 is an open specification of a graph-based effort to offer a standardized
...
Graph500 is different with other large-scale
benchmarking evaluations such as HPL and HGPGMG which primarily focus on computation
and data access patterns. Graph500 performs a breadth-first search in
parallel on a large randomly generated undirected graph. Our experiments use a
popular open source project OpenSHMEM Benchmark (OSB) suite~\cite{g500shmem} including
both MPI and OpenSHMEM based Graph500 implementations.

\subsection{MPI application}
For MPI application we use \textbf{\emph{mpi\_test\_simple}}, which is the baseline implementation that
uses two-sided \mpifunc{Send} and \mpifunc{Recv} and \mpifunc{AllReduce} to implement a breadth-first search (BFS).
We evaluate the noise overhead incurred of heartbeats messages with different heartbeat
periods on the point-to-point and collectives.
For the experiment setting we use Cori with 1K nodes. For the application setting we use \texttt{\bf $scale\_factor = 20$, $edge\_factor = 16$ }
which will generate a undirected graph with \texttt{\bf $2^{scale\_factor}$} vertices and
\texttt{\bf $2^{scale\_factor}*edge\_factor$} edges. The benchmark collects the statistics
of the generation of the breadth-first search tree of 64 randomly selected vertices. Also
collect the statistics of validation time which ensures that all connected components are visited
which generate large amount of communications.

\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{bfs_mean.pdf}
  \caption{Overhead for generating BFS running mpi\_test\_simple upon \prrte with fault tolerance over \prrte.}
  \label{fig:mean_bfs}
\end{figure}

Figure~\ref{fig:mean_bfs} shows the overhead incurred with the P2P communication during the BFS generation phase.
...
We calculate the overhead the same as in equation(1). For comparison we plot the error bars for with different $\delta$ values.
We can see under all cases the variability shows no difference as the detector is inactive.
Figure~\ref{fig:validate_bfs} illustrates the overhead incurred with \mpifunc{AllReduce} during the validation phase.
We achieve the same performance which shows that our failure detection heartbeats show
barely no impact for the application with both P2P and collective communications.

\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{validate_bfs.pdf}
  \caption{Overhead for validating BFS running mpi\_test\_simple upon \prrte with fault tolerance over \prrte.}
  \label{fig:validate_bfs}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{Overhead for generating BFS running graph500\_shmem\_one\_sided upon \prrte with fault tolerance over \prrte.}
  \label{fig:shmem_mean_bfs}
\end{figure}

\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{shmem_validate_bfs.pdf}
  \caption{Overhead for validating BFS running graph500\_shmem\_one\_sided upon \prrte with fault tolerance over \prrte.}
  \label{fig:shmem_validate_bfs}
\end{figure}
For the OpenSHMEM application we select the implementation of \textbf{\emph{graph500\_shmem\_one\_sided}} that
is using \mpi-2 one-sided code base.
For the communication it uses shmem\_put/getmem which are similar to MPI\_put/get,
and it also uses a shmem reduce collective instead of \mpifunc{AllReduce}.
Figure~\ref{fig:shmem_mean_bfs} and Figure~\ref{fig:shmem_validate_bfs} shows the overhead of
those two types of communications during BFS generating and validating.
They show similar trends that with all different heartbeat periods our detector will not stress the applications' communication.
