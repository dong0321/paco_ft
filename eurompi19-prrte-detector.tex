...
\textbf{\textit{N}}: value from environment \Comment{Number of nodes}
\textbf{\textit{Eid}}: parameter \Comment{Identifier of a process observed as failed}
\textbf{Reported$_i$}: initially empty \Comment{Set of failed process identifiers previously reported, local to daemon \textbf{i}}
\textbf{\textit{msg}}: initially empty \Comment{Message containing the set of failure affected process identifiers to report}
\textbf{Hosted\{$Did$\}}: initially empty, obtained from environment \Comment{Set of process identifiers hosted by
the daemon with identifier $Did$, empty if $Did$ is an application process}
