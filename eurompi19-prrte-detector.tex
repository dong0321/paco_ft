...
For MPI application we use "mpi\_test\_simple", which is the baseline implementation that
...
We calculate the overhead the same as in equation \eqref{eq:1}. For comparison we plot the error bars for with different $\delta$ values.
...
For the OpenSHMEM application we select the implementation of "graph500\_shmem\_one\_sided" that is using \mpi-2 one-sided code base.
%\todo{MPI_Win_Fence p2p or not?}.
