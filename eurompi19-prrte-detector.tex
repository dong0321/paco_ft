...
In this section we investigate application supports of \ourwork using different programming models
including Pmix based, \mpi and OpenMP based applications.
Figure~\ref{fig:prrte_with_oshmem} shows the overflow of building and running separate
programming models running with \ourwork:
\begin{enumerate}
...
\end{enumerate}
For all different type of applications we use \textbf{\emph{prte}} to start the DMV and the use \textbf{\emph{prun}} to execute the
application.
...
with both MPI and OpenSHMEM applications using a heavily communication-bound benchmark Graph500~\cite{graph500}.
...
For the experiment setting we use \texttt{\bf $scale\_factor = 20$, $edge\_factor = 16$ }
