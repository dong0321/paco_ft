...
\textbf{DataSpaces and FTI} \todo{add text}
 ...
 Fault Tolerance Interface (FTI) provides fast and efficient multilevel check-pointing functionality. It is an application level check-pointing offers direct data interface allows users to decide what data sets need to be protected and avoid dealing with files and/or directory names. It allow users to dedicate one process per node to overlap fault tolerance workload and computation, so that post-checkpoint tasks are executed asynchronously.
