...
We added three new components to \prrte
\todo{WIP}
...
The reliable broadcast feature is implemented in group communication and propagation component.

\todo{talk about 'OBTAIN HOSTED(i)'}
...
\begin{figure}[h]
  ...
  \label{fig:accuracy}
\end{figure}
\todo{please check if this figure is useful? I set 10s delay when starting the processes, then launching processes will not delay and cause timeout, however daemon also takes care of all process finalize which will cause a delay of timeout, because DVM still running after process finalize}
...
detection timeout $\eta$; (2) Verify that no failure is detected when there is no injection, and all injected failures are reported; (3) If the previous test is accurate, decrease $\eta$ (and accordingly the heartbeat period $\delta$) until we notice false positive detection. We set a constant ratio $\eta = \delta * 2$.
 ...
 non-communicating benchmarks (SETNAME\todo{name}), all tests succeed 
 ...
 (\imb point-to-point and collective tests), all tests succeed until the 
  ...
  congestion nor compute pressure but we found out that daemons need some time to launch the processes when starting the job which causes false detection during job startup. \todo{redo this experiment starting the detector with a large initial timeout and start injection after some cooldown period}
...
\end{equation}
 From the graph we can see that the latency performance and bandwidth performance are barely affected by heartbeats periods from milliseconds to seconds. Notably, when $ \delta \geq 10 ms $, it has trivial influence on the system, as illustrated by the fact that the average overhead is within the band of natural variability of the benchmark. When  $ \delta = 1 ms $ the noise in PingPong incurs less than three percent overhead. In collective communication the noise overhead is less than eight percent, under the circumstance that the standard deviation of the benchmark itself is four percent. 
 \todo{from here}
 Also we calculate the overhead of \ulfm with fault tolerance compared to \ulfm:
 \begin{equation}
Overhead = \frac{ \ulfm\ with\ FT - \ulfm }{\ulfm}
\end{equation}
 We can see that \ulfm with fault tolerance shows the same behavior as \ourwork. For P2P communication, when $ \delta \geq 10 ms $ which bring incidental overhead no more than two percent beyond the benchmark error, with $ \delta = 1 ms $ the noise from heartbeat emissions is around three percent. As for collective communication the overhead is about six percent with $ \delta = 1 ms $.
 \todo{to there, the results needs updated and text updated accordingly}

\subsection{Comparison with SWIM}
This section compares failure detection latency and scalability of \ourwork with SWIM~\cite{Abhinandan02} --a random-probing based failure detection protocol and gossip membership updates via network multi-cast. To decrease 
the chance of false detection, SWIM uses a suspicion mechanism. When a node does not reply direct or indirect probing in time, the initiator then judges this node as suspicious instead of failure, then broadcasts
\begin{figure}[h]
  \centering
...
\end{figure}
this suspicious information within the subgroup, if any node in the subgroup receives an acknowledge before the timeout, it will unmask the suspected node as alive, otherwise declare a failure. In order to improve the efficiency of multi-cast, SWIM uses an infection-style dissemination mechanism where information spreads in a manner analogous to the spread of gossip in society, or epidemic in the general population.

Figure \ref{fig:hb_prrte_swim} compares single node failure detection and propagation latency between \ourwork and SWIM. For the SWIM implementation, we use Go-Memberlist (\#a8f83c6). We used a go-\mpi binding to replicate 
our detection benchmark with SWIM. SWIM reports failure through Go-Memberlist callback functions. We could run Memberlist tests only up to 256 members, after that limit, some nodes exceed the maximum connection backlog 
set in the operating system for \texttt{listen} operations on TCP sockets, causing an application crash.
