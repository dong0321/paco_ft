...
\todo{ check if this is a better explanation? }
Gossip-style~\cite{infection-style,Abhinandan02} dissemination mechanisms emulate the spread of gossip in society. Initially, members are inactive except one member is aware of failure and need to propagate this information. It propagates this information by randomly ping other members until it pings someone who already get notified. Notified members use the same strategy to distribute information. Gossip-style is resilient to process failure and spreads exponentially quickly in the group, however the worst case could be some members never get notified.   
